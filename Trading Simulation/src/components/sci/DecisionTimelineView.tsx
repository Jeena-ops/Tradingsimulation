...
export function DecisionTimelineView() {
  ...
  const getActionFullLabel = (action: Decision['action']) => {
    if (language === 'zh') {
      switch (action) {
        case 'buy': return '买入';
        case 'sell': return '卖出';
    ...
    <div className="p-6 space-y-6">
      ...
      <div>
        <h1 className="text-white text-2xl mb-2">
          {language === 'zh' ? '决策时序分析' : 'Decision Timeline Analysis'}
        </h1>
        <p className="text-gray-400">
          {language === 'zh' 
            ? '24小时AI决策矩阵热力图 · 每小时观察决策循环' 
            : '24-Hour AI Decision Matrix Heatmap · Hourly Decision Cycle'}
        </p>
      </div>
      ...
      <div className="glass-card rounded-lg p-6">
        <h3 className="text-white mb-4">
          {language === 'zh' ? '24小时决策时间轴' : '24-Hour Decision Timeline'}
        </h3>
                ...
                <Tooltip>
                  ...
                  <TooltipContent>
                    <div className="text-xs">
                      <div>{language === 'zh' ? '买入' : 'Buy'}: {stats.buy}{language === 'zh' ? '次' : ''}</div>
                      <div>{language === 'zh' ? '卖出' : 'Sell'}: {stats.sell}{language === 'zh' ? '次' : ''}</div>
                      <div>{language === 'zh' ? '持有' : 'Hold'}: {stats.hold}{language === 'zh' ? '次' : ''}</div>
                    </div>
                  </TooltipContent>
                </Tooltip>
              </TooltipProvider>
            ...
            <div className="flex items-center justify-between">
              <h4 className="text-white">
                {language === 'zh' 
                  ...
                  : `${selectedHour}:00 Decision Matrix`}
              </h4>
              <div className="flex items-center gap-4 text-sm">
                <div className="flex items-center gap-2">
                  <div className="w-3 h-3 bg-red-500 rounded" />
                  <span className="text-gray-400">{language === 'zh' ? '买入' : 'Buy'}</span>
                </div>
                <div className="flex items-center gap-2">
                  <div className="w-3 h-3 bg-green-500 rounded" />
                  <span className="text-gray-400">{language === 'zh' ? '卖出' : 'Sell'}</span>
                </div>
                <div className="flex items-center gap-2">
                  <div className="w-3 h-3 bg-gray-500 rounded" />
                  <span className="text-gray-400">{language === 'zh' ? '持有' : 'Hold'}</span>
                </div>
              </div>
            </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-700">
                    <th className="text-left text-gray-400 p-2 sticky left-0 bg-[#111827]">
                      {language === 'zh' ? 'AI模型' : 'AI Model'}
                    </th>
                                ...
                                {decision && (
                                  <TooltipContent>
                                    <div className="text-xs space-y-1">
                                      ...
                                      <div>{language === 'zh' ? '操作' : 'Action'}: {getActionFullLabel(decision.action)}</div>
                                      <div>{language === 'zh' ? '价格' : 'Price'}: ¥{decision.price.toFixed(2)}</div>
                                      <div>{language === 'zh' ? '置信度' : 'Confidence'}: {decision.confidence.toFixed(1)}%</div>
                                    </div>
                                  </TooltipContent>
                                )}
                              </Tooltip>
      ...
      {selectedHour !== null && hourlyDecisions.length > 0 && (
        <div className="glass-card rounded-lg p-6">
          <h3 className="text-white mb-4">
            {language === 'zh' 
              ? `${selectedHour}:00 详细决策记录` 
              : `${selectedHour}:00 Detailed Decisions`}
          </h3>
          <div className="grid grid-cols-2 gap-4">
            {hourlyDecisions.slice(0, 6).map((decision, idx) => {
              const ai = AI_MODELS.find(m => m.id === decision.aiModelId);
              const actionConfig = {
                buy: { label: language === 'zh' ? '买入' : 'Buy', color: 'bg-red-600', icon: TrendingUp },
                sell: { label: language === 'zh' ? '卖出' : 'Sell', color: 'bg-green-600', icon: TrendingDown },
                hold: { label: language === 'zh' ? '持有' : 'Hold', color: 'bg-gray-600', icon: Activity },
              }[decision.action];
                ...
                <div key={idx} className="p-4 bg-gray-700 bg-opacity-30 rounded-lg">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-gray-400">{language === 'zh' ? '股票' : 'Stock'}</span>
                      <span className="text-white">{decision.stockCode}</span>
                    </div>
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-gray-400">{language === 'zh' ? '价格' : 'Price'}</span>
      ...
      <div className="grid grid-cols-4 gap-4">
        <div className="glass-card rounded-lg p-4">
          <div className="text-sm text-gray-400 mb-2">
            {language === 'zh' ? '今日总决策数' : 'Total Decisions'}
          </div>
          <div className="text-2xl text-white">{decisionMatrix.size}</div>
        </div>
        <div className="glass-card rounded-lg p-4">
          <div className="text-sm text-gray-400 mb-2">
            {language === 'zh' ? '买入信号' : 'Buy Signals'}
          </div>

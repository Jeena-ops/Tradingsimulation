    ...
    <div className="p-6 space-y-6">
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className="text-white text-xl mb-1">
              {language === 'zh' ? 'AI模型收益对比' : 'AI Model Performance Comparison'}
            </h2>
            <p className="text-sm text-gray-400">
              {language === 'zh' ? '实时收益率走势（过去24小时）' : 'Real-time Return Trends (Last 24 Hours)'}
            </p>
          </div>
          <div className="flex items-center gap-2">
            <TrendingUp className="w-5 h-5 text-green-500" />
            <span className="text-white">{language === 'zh' ? '实时更新' : 'Live Update'}</span>
          </div>
        </div>
      ...
      <div>
        <h2 className="text-white text-xl mb-4">
          {language === 'zh' ? 'AI模型监控面板' : 'AI Model Dashboard'}
        </h2>
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        <div className="glass-card rounded-lg p-4">
          <div className="text-sm text-gray-400 mb-2">
            {language === 'zh' ? '运行中模型' : 'Active Models'}
          </div>
          <div className="text-2xl text-white">
        ...
        <div className="glass-card rounded-lg p-4">
          <div className="text-sm text-gray-400 mb-2">
            {language === 'zh' ? '总持仓数' : 'Total Positions'}
          </div>
          <div className="text-2xl text-white">
